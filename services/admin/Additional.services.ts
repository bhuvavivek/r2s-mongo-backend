...
class AdditionalService {
    ...
    read = async (cond = {}, select = {
        ...
        contact_no: true,
        contact_email: true,
        Master_referall_amount: true,
        Child_referall_amount: true,
        SubChild_referall_amount: true,
        discount: true,
        createdAt: true,
        updatedAt: true
    }) => {
        try {
            const result = await this.prisma.additionalDetail.findFirst({
                where: { ...cond },
            });
            return result;
        } catch (err) {
    ...
    update = async (id: string, data: object) => {
        try {
            const result = await this.prisma.additionalDetail.update({
                where: { id },
                data,
            });
            return result;
